/* @author AZbang
 * @version  0.2.0
 * @link https://github.com/AZbang/RadialBar
 * @license MIT
*/
var RadialBar =
...
/***/ function(module, exports, __webpack_require__) {
	...
	class RadialBar {
		...
		add(p) {
			if (!this.isStop) {
				...
				if (this.progress >= 100) {
					...
					window.dispatchEvent(new Event("endProgress"));
					this.isStop=true;
				}
			}
		}
		...
		showInfoText(p) {
			...
			this.ctx.fillText(this.title, this.x, this.y)
			//this.ctx.fillText(this.get() + '%', this.x, this.y+20);
			this.ctx.restore();
		}
		...
		update() {
			...
			this.ctx.beginPath();
			this.ctx.arc(0, 0, this.radius+this.lineWidth/2, 0, 2 * Math.PI);
			this.ctx.fill();
